...
Deno.serve(async (req) => {
  ...
  if (!ownerId && !messageId && !connectionId) {
    return json({ error: 'missing_filter', hint: 'provide owner_id, message_id or connection_id' }, 400);
  }

  const sinceIso = new Date(Date.now() - sinceHours * 3600 * 1000).toISOString();

  // 1. Discover WAHA connections in scope
  let connQuery = admin.from('whatsapp_connections')
    ...
    .eq('provider', 'waha');
  if (ownerId) connQuery = connQuery.eq('owner_id', ownerId);
  ...
  if (connIds.length === 0 && !messageId) {
    return json({
      ok: true, owner_id: ownerId, connections: [], events: [], messages: [],
      gaps: [], stats: { events_total: 0, message_events: 0, messages_stored: 0, gaps: 0 },
    });
  }
  ...
  let msgQ = admin.from('chat_messages')
    ...
    .order('created_at', { ascending: false })
    .limit(limit);
  if (ownerId) msgQ = msgQ.eq('customers.owner_id', ownerId);
  ...
  // 4. Compute gaps: message-bucket events whose provider_msg_id is missing from chat_messages
  ...
  const gaps = (events ?? [])
    ...
    .map((e: any) => ({
      ...
      sender_lid: e.metadata_json?.sender_lid,
      sender_jid: e.metadata_json?.sender_jid,
      owner_id: e.metadata_json?.owner_id,
      raw_event: e.metadata_json?.raw_event,
    }));
  ...
  // 5. Telemetry alert flags — surfaced so the UI/log ingestion can raise a red badge.
  const alerts: string[] = [];
  ...
  // Best-effort: persist gap telemetry so it drives the shared alert timeline.
  if (gaps.length > 0) {
    try {
  ...
  return json({
    ...
    connections, events, messages, gaps, stats, alerts,
  });
});
