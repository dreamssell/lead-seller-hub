...
BEGIN
  ...
  -- Validation for closing transitions: must have an open attendance to close.
  IF p_stage = 'closed' THEN
    IF v_existing_id IS NULL THEN
      SELECT closed_at INTO v_last_closed_at
       ...
       LIMIT 1;
      IF v_last_closed_at IS NOT NULL THEN
        RAISE EXCEPTION 'already_closed' USING
          MESSAGE = 'Este atendimento já está finalizado.',
          HINT = 'already_closed';
      ELSE
        RAISE EXCEPTION 'no_open_attendance' USING
          MESSAGE = 'Não há atendimento aberto para encerrar.',
          HINT = 'no_open_attendance';
      END IF;
    END IF;
  END IF;
  ...
  -- Bump customers.updated_at to trigger realtime for observers (e.g. CollaborationBar).
